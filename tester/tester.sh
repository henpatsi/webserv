...
curl http://$SERVER_IP:$SERVER_PORT/html/success.html
echo ""

echo "---Test simple POST---"
curl -d "message=Hello, World!&name=mom" http://$SERVER_IP:$SERVER_PORT/html/success.html
echo ""

echo "---Test simple POST with false line length---"
curl -H "Content-Length: 100" -d "message=Hello, World!&name=mom" http://$SERVER_IP:$SERVER_PORT/html/success.html
echo ""

echo "---Test chunked transfer---"
curl -H "Transfer-Encoding: chunked" -d @$TEST_MESSAGE_DIR/hhgttg_short.txt http://$SERVER_IP:$SERVER_PORT/html/success.html
echo ""

echo "---Test chunked transfer empty---"
curl -H "Transfer-Encoding: chunked" -d "" http://$SERVER_IP:$SERVER_PORT/html/success.html
echo ""
...
curl -H "Key:value" http://$SERVER_IP:$SERVER_PORT/html/success.html
echo ""
...
curl -H "Key: value space" http://$SERVER_IP:$SERVER_PORT/html/success.html
echo ""

echo "---Test Space In Key1---" # NGINX does not allow
curl -H "Key space: value" http://$SERVER_IP:$SERVER_PORT/html/success.html
echo ""
...
curl -H "Key:" http://$SERVER_IP:$SERVER_PORT/html/success.html
echo ""
...
curl -H ": asdf" http://$SERVER_IP:$SERVER_PORT/html/success.html
echo ""
